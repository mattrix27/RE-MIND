...
class Houseworks():
	...
	def Check_Sched(self, groups, start_row, start_col):
				...
				if datez != '':				
					...
					if datez[2] == today[0][2:] and int(today[1]) == int(datez[0]):
						if int(datez[1]) == int(today[2])-1:
							...
							da_time = self.sheet.cell(i, start_col+1).value
							time = datetime.datetime.now()
							if da_time[0] == time.hour+4 or da_time[0] == time.hour +1:
					...
					check_dates = False
houseworks = Houseworks('FB')
groups = houseworks.GROUPON(2,2)
houseworks.Check_Sched(groups, 1, 6)
